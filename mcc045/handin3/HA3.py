...
L_obj_ocu= ; % distance from objective lens to ocular lens *CODE MISSING*
...
size_of_numerical_window_retina=200e-6; % a "biologically" sensible default value if N=256 (the detector spacing in the retina is >= 1µm 
...
transmission_function_eyelens= ; % *CODE MISSING*


%************** Initiate the plane of the retina
a_retina= ; % sampling distance *CODE MISSING*
...
for point_source_number=1:number_of_point_sources_in_simulation % one point source at a time
    ...
    r_center= ; % distance from point source to the origin (0,0) in the plane of the objective lens *CODE MISSING*
    E_obj_in= ; % incident field in (the sampling points in) the plane of the objective lens, from the point source *CODE MISSING*
    E_obj_out= ; % field in the plane after the objective lens *CODE MISSING*
    ...
    [E_ocu_in,L1,L2,uvect,vvect,umat,vmat]= ; % field in the plane before the ocular lens *CODE MISSING*
    E_ocu_out= ; % field in the plane after the ocular lens *CODE MISSING*
    ...
    [E_eye_in,L1,L2,uvect,vvect,umat,vmat]= ; % field in the plane before the pupil/eye lens *CODE MISSING*
    E_eye_out= ; % field in the plane after the pupil/eye lens *CODE MISSING*
    I_eye_out= ;% Just to irritate you, you will need to fill it in yourself this time :) 
    ...
    [E_retina,L1,L2,uvect,vvect,umat,vmat]= ; % field in the plane at the retina *CODE MISSING*
    ...
    I_retina_total=I_retina_total + ; % the sum of intensity distributions from all point sources considered so far *CODE MISSING*
    figure(6)
