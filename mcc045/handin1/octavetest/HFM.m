function [E2, L1, L2, uvect, vvect, umat, vmat] = HFM(E1, a, b, L, lbda_0, n)
    # This function implements the conventional Huygens-Fresnel method (HFM) for free space propagation
    ...
    fctr1 = exp(-1j*k * (umat.^2 + vmat.^2) / (2*L2));
    prefactor = exp(1j*k * (xiMat.^2 + etaMat.^2) / 2 * (1/L1 - 1/L2));
    fft_part = fft2c(E1.*exp(1j*k * (xmat.^2 + ymat.^2) / (2*L1)));
